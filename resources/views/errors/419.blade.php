  ...
  <head>
    ...
    <style>
      ...
      .secondary-text-color {
        color: #8898f2 !important;
      }

      .border-button {
        color: #F2F2F2 !important;
        border-color: #8898f2 !important;
      }
      .button {
  ...
  <body class="plain error-page-wrapper background-color background-image">
    <div class="content-container">
      ...
      <div class="subheader primary-text-color">
        {!! __('errorpage.419_title') !!}
      </div>
      <hr />
      <div class="clearfix"></div>
      <div class="context primary-text-color">
        <p>
          {{__('errorpage.419_content')}}
        </p>
      </div>
      <div class="buttons-container">
        <a
          class="border-button"
          href="mailto:ppare.vatsim@gmail.com"
          target="_blank"
          >{{__('errorpage.419_report')}}</a
        >
      </div>
    </div>
    ...
    <script>
      function ErrorPage(container, pageType, templateName) {
        ...
        this.$contentContainer = this.$container.find(
          templateName == 'sign' ? '.sign-container' : '.content-container'
        );
      ...
      ErrorPage.prototype.handleStatusFetchSuccess = function (pageType, data) {
        if (pageType == '419') {
          ...
          if (!!data.scheduled_maintenances.length) {
            ...
            $('#replace-with-fetched-data').html(
              this.createTimeRangeTag(
                maint.scheduled_for,
                maint.scheduled_until
              )
            );
            $.fn.localizeTime();
          } else {
      ...
      ErrorPage.prototype.handleStatusFetchFail = function (pageType) {
        $('#replace-with-fetched-data').html(
          '<em>(enter a valid Statuspage url)</em>'
        );
      };
      ...
      var ep = new ErrorPage('body', '419', 'plain');
      ep.initialize();
