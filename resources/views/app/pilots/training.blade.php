...
@section('page-title')
  Pilot Training Center | {{ Auth::user()->fname }}
@endsection
...
@section('page-header')
  ...
  <section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1>{{__('app/pilot/pilot_training_center.header_title', ['FNAME' => Auth::user()->fname])}}</h1>
        </div>
      </div>
    </div><!-- /.container-fluid -->
  </section>
...
<div class="container-fluid">
    ...
    <div class="col-md-3">
      <div class="small-box bg-success elevation-3">
        <div class="inner">
          ...
          <p>{{__('app/pilot/pilot_training_center.ment_sessions')}}</p>
        </div>
        <div class="icon">
    ...
    <div class="col-md-3">
      <div class="small-box bg-warning elevation-3">
        <div class="inner">
          ...
          <p>{{__('app/pilot/pilot_training_center.teaching_p')}}</p>
        </div>
        <div class="icon">
    ...
    <div class="col-md-3">
      <div class="small-box bg-info elevation-3">
        <div class="inner">
          ...
          <p>{{__('app/pilot/pilot_training_center.curr_ment')}}</p>
        </div>
        <div class="icon">
          ...
          <div class="row">
            <div class="col-md-6">
              <button type="button" class="btn btn-flat btn-info" data-toggle="modal" data-target="#request_session">
                {{__('app/pilot/pilot_training_center.req_session')}}
              </button>
              
            </div>
            <div class="col-md-6">
              <button type="button" class="btn btn-flat btn-info float-right" data-toggle="modal" data-target="#send_message">
                {{__('app/pilot/pilot_training_center.send_msg_btn', ['MENTOR' => $mentorObj->fname])}}
              </button>
            </div>
          </div>
        </div>
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title">{{__('app/pilot/pilot_training_center.title_req', ['MENTOR' => $mentorObj->fname." ".$mentorObj->lname])}}</h4>
        ...
        <form action="{{ route('app.pilot.training.requestsession', app()->getLocale()) }}" method="post">
            ...
            <div class="row">
              <div class="col-md-4">
                <div class="form-group">
                  <label for="session-date">{{__('app/pilot/pilot_training_center.date')}}</label>
                  <input type="text" class="form-control" id="session-date" name="sessiondate" placeholder="{{__('app/pilot/pilot_training_center.date')}}">
                </div>
              </div>
              <div class="col-md-4">
                <div class="form-group">
                  <label for="starttime">{{__('app/pilot/pilot_training_center.st_time')}} (UTC)</label>
                  <input type="text" class="form-control" id="starttime" name="starttime" placeholder="{{__('app/pilot/pilot_training_center.st_time')}}">
                </div>
              </div>
              <div class="col-md-4">
                <div class="form-group">
                  <label for="endtime">{{__('app/pilot/pilot_training_center.end_time')}} (UTC)</label>
                  <input type="text" class="form-control" id="endtime" name="endtime" placeholder="{{__('app/pilot/pilot_training_center.end_time')}}">
                </div>
              </div>
            </div>
            <div class="form-group">
          ...
          <div class="modal-footer justify-content-between">
            ...
            <button type="submit" class="btn btn-success">{{__('app/pilot/pilot_training_center.send_req')}}</button>
          </div>
        </form>
      </div>
    </div>
  ...
  <div class="modal fade" id="send_message">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title">{{__('app/pilot/pilot_training_center.title_msg', ['MENTOR' => $mentorObj->fname." ".$mentorObj->lname])}}</h4>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <form action="{{ route('app.inmsg.send', app()->getLocale()) }}" method="post">
          ...
          <div class="modal-body">
            <div class="form-group">
              <label for="msgsubject">{{__('app/pilot/pilot_training_center.subject')}}</label>
              <input type="text" class="form-control" id="msgsubject" name="msgsubject" placeholder="{{__('app/pilot/pilot_training_center.subject')}}">
            </div>
            <div class="form-group">
              <label for="msgbody">{{__('app/pilot/pilot_training_center.msg')}}</label>
              <textarea class="form-control" rows="15" name="msgbody" id="msgbody" placeholder="{{__('app/pilot/pilot_training_center.msg_placeholder')}}"></textarea>
            </div>
          </div>
          <input type="hidden" name="msgrecipient" value="{{ $mentorObj->id }}">
          <div class="modal-footer justify-content-between">
            <button type="button" class="btn btn-default" data-dismiss="modal">{{__('app/pilot/pilot_training_center.close')}}</button>
            <button type="submit" class="btn btn-success">{{__('app/pilot/pilot_training_center.send_msg')}}</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <div class="col-md-12">
      ...
      <div class="card card-dark elevation-3">
        <div class="card-header">
          <h3 class="card-title">{{__('app/pilot/pilot_training_center.upcoming_sess')}}</h3>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
          <table
            ...
            <tr>
              ...
              <th>{{__('app/pilot/pilot_training_center.mentor_comment')}}</th>
              <th>{{__('app/pilot/pilot_training_center.student_comment')}}</th>
              <th>{{__('app/pilot/pilot_training_center.status')}}</th>
              <th>{{__('app/pilot/pilot_training_center.options')}}</th>
            </tr>
            </thead>
            <tbody>
              @foreach ($sessions as $training)
                    ...
                    @else
                      ...
                      @else
                        ...
                        @else
                          ...
                          {{__('app/pilot/pilot_training_center.no_report')}}

                          @else

                            {{-- Training completed, has report --}}
                            <button type="button" class="btn btn-block btn-info btn-flat" data-toggle="modal" data-target="#mentor_report_{{ $training['id'] }}">{{__('app/pilot/pilot_training_center.see_report')}}</button>
                ...
                <div class="modal fade" id="student_comment_{{ $training['id'] }}">
                  <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                      <div class="modal-header">
                        <h4 class="modal-title">{{__('app/pilot/pilot_training_center.student_comment')}}</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                          <span aria-hidden="true">&times;</span>
                        </button>
                      </div>
                ...
                <div class="modal fade" id="mentor_report_{{ $training['id'] }}">
                  <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                      <div class="modal-header">
                        <h4 class="modal-title">{{__('app/pilot/pilot_training_center.mentor_report')}}</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                          <span aria-hidden="true">&times;</span>
                        </button>
                      </div>
...
<script>
  $('#training_sessions').DataTable({
    ...
    "info": false,
    "language": {
      "emptyTable" : "{{__('app/pilot/pilot_training_center.empty_table')}}"
    }
  });
